...
#os.system('nvidia-smi --query-gpu=power.draw,utilization.gpu,utilization.memory,temperature.gpu,pstate, --format=csv,nounits --id=0 -lms 500 -f ./profiles/GPUprofile_double_unet_t4_tr.csv &') 
...
#print("Epoch training time: {0}".format((end_time-start_time)/32))


start_time = time.time()
#os.system('nvidia-smi --query-gpu=power.draw,utilization.gpu,utilization.memory,temperature.gpu,pstate, --format=csv,nounits --id=0 -lms 500 -f ./profiles/GPUprofile_double_unet_t4_ts.csv &')
#model.evaluate(test_x, test_y, batch_size=8) #callbacks=[cp_callback, ev_callback])
#os.system('kill $(ps aux | grep \'nvidia-smi\' | awk \'{print $2}\')')
end_time = time.time()

print("Evaluation time: {0}".format((end_time-start_time)/32))
