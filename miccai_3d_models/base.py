...
def train_cascaded_model(model, train_x_data, train_y_data, options):
    ...
    checkpoint_path = "models/model1_full.ckpt"
    ...
    checkpoint_path = "models/model2_full.ckpt"
    checkpoint_dir = os.path.dirname(checkpoint_path)
    ...
    history2 = model[1].fit(X, Y, batch_size=options['batch_size'], epochs = options['max_epochs'], validation_split=options['train_split'], verbose=options['net_verbose'], callbacks = [earlystop_callback, cp_callback])
                            #, callbacks= [earlystop_callback]) #model[1] or model[0]?
...
def test_scan(model, test_x_data, options, save_nifti= True, candidate_mask = None):
    ...
    # TODO: This is why we have to add sigmoid at the final layer!
...
#         y_pred = model.predict_proba(np.squeeze(batch))
        y_pred = probability_model.predict_proba(batch)
